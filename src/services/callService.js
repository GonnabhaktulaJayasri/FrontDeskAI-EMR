...
//                 ext.url === 'http://hospital-system/twilio-phone'
//             );
...
class CallService {
    ...
    async makeOutboundCall(options) {
        ...
        try {
            ...
            // Get Twilio phone number from hospital - check multiple sources
            let hospitalTwilioNumber = null;
            
            // First, check identifier (primary method)
            const twilioIdentifier = hospital.identifier?.find(id => 
                id.system === 'http://hospital-system/twilio-phone'
            );
            
            if (twilioIdentifier && twilioIdentifier.value) {
                hospitalTwilioNumber = twilioIdentifier.value;
                console.log('Found Twilio number in identifier:', hospitalTwilioNumber);
            }
            
            // Fallback 1: check extension with original URL
            if (!hospitalTwilioNumber) {
                const twilioPhoneExt = hospital.extension?.find(ext => 
                    ext.url === 'http://hospital-system/twilio-phone'
                );
                if (twilioPhoneExt && twilioPhoneExt.valueString) {
                    hospitalTwilioNumber = twilioPhoneExt.valueString;
                    console.log('Found Twilio number in extension (twilio-phone):', hospitalTwilioNumber);
                }
            }
            
            // Fallback 2: check extension with alternate URL
            if (!hospitalTwilioNumber) {
                const twilioPhoneExt2 = hospital.extension?.find(ext => 
                    ext.url === 'http://hospital-system/twilio-phone-number'
                );
                if (twilioPhoneExt2 && twilioPhoneExt2.valueString) {
                    hospitalTwilioNumber = twilioPhoneExt2.valueString;
                    console.log('Found Twilio number in extension (twilio-phone-number):', hospitalTwilioNumber);
                }
            }

            if (!hospitalTwilioNumber) {
                console.error('Hospital data:', JSON.stringify(hospital, null, 2));
                throw new Error("Hospital does not have a Twilio phone number configured. Please add it via identifier or extension.");
            }
            
            console.log('Using Twilio number:', hospitalTwilioNumber);
            let patient = null;
